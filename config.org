...
Splash screen setup
#+BEGIN_SRC emacs-lisp
(setq +doom-dashboard-banner-file "/opt/homebrew/opt/emacs-plus@31/share/emacs/31.0.50/etc/images/splash.png")
#+END_SRC
...
(after! bibtex
  ...
  ;; bibtex-completion settings (citar delegates to bibtex-completion internally)
  (setq bibtex-completion-pdf-field "file"
        bibtex-completion-library-path `(,my/pdf-directory)
        bibtex-completion-bibliography my/bibliography
        bibtex-completion-additional-search-fields '(keywords doi))
...
(use-package! citar
  ...
  :config
  ;; Indicators for has-PDF / has-note / has-link in the completion UI.
  ;; Deferred to :config so all-the-icons is loaded before we call its fns.
...
;; Performance tuning for LSP / R languageserver
(setq read-process-output-max (* 1024 1024)) ;; 1MB pipe buffer; improves LSP throughput
(after! eglot
  (setq eglot-events-buffer-size 0    ;; don't log all LSP traffic (reduces GC pressure)
        eglot-autoshutdown t          ;; kill R LSP process when last buffer closes
        eglot-sync-connect 1          ;; 1s then connect in background; avoids startup freeze
        eglot-ignored-server-capabilities
        '(:documentOnTypeFormattingProvider))) ;; disable on-type formatting; ESS handles indentation
...
(defun my/r-lsp ()
  "Start eglot in actual R files (but not poly-mode inner blocks)"
  (interactive)
  (let ((buf (buffer-file-name)))
    (when (and buf
               (member (downcase (file-name-extension buf)) '("r")))
      (eglot-ensure))))
...
#+BEGIN_SRC emacs-lisp
;; C-<arrow> handles windmove (see local_post.el).
;; C-M-<left>/<right> cycles between frames (forward/backward).
  (global-set-key [C-M-right] #'other-frame)
  (global-set-key [C-M-left]  (lambda () (interactive) (other-frame -1)))
#+END_SRC
