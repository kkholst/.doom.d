...
(defvar my/opencmd
  (cond
   ...
   ((and (eq system-type 'gnu/linux)
    ...
    ;; requires wslpath -- sudo apt install wslu
    "wslview")
   ;; Check for macOS
   (IS-MAC "open")
   ;; Default to xdg-open on other Linux/Unix systems
   (t "/usr/bin/xdg-open"))
  "Program for opening files and applications")
#+END_SRC
...
Splash screen setup
#+BEGIN_SRC emacs-lisp
(setq +dashboard-banner-dir doom-private-dir
      +dashboard-banner-file "mascot.png")
#+END_SRC
...
#+END_SRC
 ...
 The following uses the frame-bar to display the buffer/filename and projectile project
#+BEGIN_SRC emacs-lisp
...
#+BEGIN_SRC emacs-lisp
  ...
  (if (file-exists-p (concat my/emacshome "local_pre.el"))
      (load (concat my/emacshome "local_pre.el")))
#+END_SRC
...
(let* ((buf (buffer-file-name))
  ...
  (async-shell-command (concat "quarto render " buf))
  ;; (ess-eval-linewise cmd)
  ;; (message (concat "Written to " out))
   )
)
(defalias 'my/qmd-export 'my/rmd-export)
#+end_src
...
;;(setq ess-eval-visibly t) ;; 'nowait
...
;; https://github.com/emacs-ess/ESS/issues/1297#issuecomment-2322831635
;; NOTE: cape-wrap-noninterruptible disables C-g during the call, which
;; causes Emacs to fully hang if the inferior R is busy (long computation,
;; readline, browser()). Keep it ONLY for ess-roxy-complete-tag (the case
;; the upstream issue actually addresses). For the others, use
;; cape-wrap-nonexclusive so corfu can fall back to other capfs.
(advice-add 'ess-roxy-complete-tag    :around #'cape-wrap-noninterruptible)
(advice-add 'ess-filename-completion  :around #'cape-wrap-nonexclusive)
(advice-add 'ess-r-package-completion :around #'cape-wrap-nonexclusive)
(advice-add 'ess-r-object-completion  :around #'cape-wrap-nonexclusive)

;; Run lintr only on save / mode-enable, not on every keystroke. lintr
;; spawns an R subprocess each run and is a major source of stalls.
(after! flycheck
  (setq flycheck-check-syntax-automatically '(save mode-enabled)
        flycheck-idle-change-delay 2.0))

(after! ess
  (map! :map inferior-ess-r-mode-map "M-r" #'consult-history))
...
(after! ess
  ...
  ;;   ;; Only spin up eglot for genuine .R/.r files, not poly-mode chunks.
  ;;  (add-hook 'ess-r-mode-local-vars-hook #'my/r-lsp 'append))

  ;; Eglot/eldoc tuning to avoid blocking the UI on completion/sig help.
  (when (boundp 'eglot-events-buffer-size)
    (setq eglot-events-buffer-size 0)) ;; less log overhead
...
(add-hook 'inferior-R-mode-hook
          (lambda ()
            ;; Slow down eldoc and don't auto-pop completion in the REPL;
            ;; ess-r-object-completion blocks Emacs while waiting for the
            ;; inferior R process. Manual TAB still works.
...
;; In R source buffers, give corfu a longer delay so a stray keystroke
;; doesn't immediately fire ess-r-object-completion against a busy R.
...
(map! :leader "c R" #'my/ess-edit-reload)
...
Disable suspend-frame key binding
#+BEGIN_SRC emacs-lisp
(use-package undo-fu
  :config
...
#+BEGIN_SRC emacs-lisp
  ...
  (map! "M-s s" #'ispell-word)
  (map! "M-s s" #'ispell-word)
  (map! "M-s M-s" #'cape-dict)
...
(map! :leader "n ?" #'biblio-doi-insert-bibtex)
(map! :leader "n b" #'citar-open) ;; instead of citar-open-notes
(map! :leader "n B" #'dissemin-lookup)
(map! :leader "! !" #'consult-flycheck)
(map! :leader "<tab>" #'yasnippet-capf)
#+END_SRC
...
*** Midnight-Commander style two-pane dired

A fast MC-like workflow on top of vanilla dired. Hit =<f9>= (or =SPC o m=) to
open the current directory in a left/right split with a second dired pane on
the right defaulting to =$HOME= (or the other window's directory if it is
already a dired buffer). =dired-dwim-target= makes =C= (copy) and =R= (rename
/ move) propose the *other* pane as destination, exactly like MC's F5/F6.

| key   | action                                                   |
|-------+----------------------------------------------------------|
| <f3>  | view file (read-only)                                    |
| <f4>  | edit file                                                |
| <f5>  | copy marked files to other pane                          |
| <f6>  | rename / move marked files to other pane                 |
| <f7>  | create directory                                         |
| <f8>  | delete marked files                                      |
| <f9>  | toggle two-pane dired                                    |
| TAB   | switch to other dired pane                               |
| =     | diff file at point against same-named file in other pane |

#+BEGIN_SRC emacs-lisp
(defun my/dired-two-pane (&optional dir)
  "Open DIR (default: current) in a left/right two-pane dired layout.
The right pane defaults to $HOME unless another dired buffer is already
visible, in which case its directory is reused."
  (interactive)
  (let* ((left  (or dir
                  ...
                  (dolist (w (window-list))
                    (with-current-buffer (window-buffer w)
                      (when (and (derived-mode-p 'dired-mode)
                                 (not (equal default-directory left)))
                        (throw 'found default-directory))))
                  nil))
         (right (or other (expand-file-name "~/"))))
    (delete-other-windows)
    (dired left)
...
(defun my/dired-toggle-two-pane ()
  "Toggle between single-pane and two-pane dired."
  (interactive)
...
(defun my/dired-other-pane-dir ()
  "Return the directory shown in the other dired window, or nil."
  (let ((w (next-window)))
...
(defun my/dired-diff-with-other-pane ()
  "Diff file at point against same-named file in the other dired pane."
  (interactive)
  (let* ((this (dired-get-filename nil t))
         (other-dir (my/dired-other-pane-dir))
         (other (and other-dir (expand-file-name (file-name-nondirectory this) other-dir))))
    (if (and other (file-exists-p other))
        (ediff-files this other)
      (user-error "No matching file in other pane"))))

(after! dired
  ;; F-keys mimic Midnight Commander
  (map! :map dired-mode-map
        ...
        "<f8>" #'dired-do-delete
        "<f9>" #'my/dired-toggle-two-pane
        "TAB"  #'other-window
        "="    #'my/dired-diff-with-other-pane))

;; Global launcher: <f9> anywhere opens the two-pane view rooted at the
;; current buffer's directory.
(map! "<f9>" #'my/dired-two-pane
      :leader "o m" #'my/dired-two-pane)
#+END_SRC

*** Sunrise Commander

A more faithful Norton/MC clone built on dired. Launch with =SPC o M= or
=M-x sunrise=. Inside Sunrise, =C-c t= opens a vterm at the active pane's
directory, =C-c C-c= clones the active pane to the passive one, and the
usual MC keys (F5/F6/F7/F8) work too.

#+BEGIN_SRC emacs-lisp
(use-package! sunrise-commander
  :commands (sunrise sunrise-cd)
  :init
  (map! :leader "o M" #'sunrise))
#+END_SRC

*** Dirvish

Modern, polished dired replacement with rich previews, icon support and
fast async listings. Launched explicitly via =SPC o d= so it co-exists
with vanilla dired, =my/dired-two-pane=, and Sunrise without taking over.

| key            | action                                |
|----------------+---------------------------------------|
| SPC o d        | open dirvish in current directory     |
| q              | quit dirvish session                  |
| TAB            | toggle file preview                   |
| a              | toggle attribute columns              |
| f              | jump to file (fd / consult)           |
| s              | sort cycle                            |
| y              | yank menu (copy/move/symlink/...)     |
| M-f / M-b      | history forward / back                |
| M-n / M-p      | next / previous file in subtree       |

Dirvish is *not* set as a global override of dired (we keep dired vanilla
to preserve =dired+=, =dired-narrow=, =peep-dired=, two-pane and Sunrise
behaviour). Use =M-x dirvish-override-dired-mode= interactively if you
want to A/B test the global takeover.

#+BEGIN_SRC emacs-lisp
(use-package! dirvish
  :commands (dirvish dirvish-side dirvish-dwim)
  :init
  (map! :leader "o d" #'dirvish)
  :config
  ;; Columns shown in the main file list.
  (setq dirvish-attributes
        '(vc-state subtree-state nerd-icons collapse git-msg file-time file-size))
  ;; Use the same listing switches as our dired setup.
  (setq dirvish-hide-details nil)
  (setq dirvish-default-layout '(0 0.4 0.6))    ; no parent, 40% main / 60% preview
  (setq dirvish-mode-line-format
        '(:left (sort symlink) :right (omit yank index)))
  ;; Reuse our preferred ls switches when available.
  (when (boundp 'dired-listing-switches)
    (setq dirvish-listing-switches dired-listing-switches)))
#+END_SRC

*** diredc

A more "commander-like" experience built closer to vanilla dired.
Provides dual-pane frames, persistent per-buffer history, trash support,
quick shell, and bookmarks. Launched via =SPC o D= so it stays opt-in.
...
| C-c ?     | show help (very useful, lots of keys)  |
| C-q       | quit diredc and clean up its frames    |
| M-d       | new dual-pane frame                    |
| C-<right> / C-<left> | history forward / back        |
| C-c C-s   | pop a shell in current directory       |
| C-c C-t   | toggle trash (use trash vs delete)     |
| C-c C-r   | rename / refactor selection            |

#+BEGIN_SRC emacs-lisp
(use-package! diredc
  :commands (diredc diredc-mode)
  :init
  (map! :leader "o D" #'diredc)
  :config
  ;; Send deletions to the system trash instead of nuking files.
  (setq diredc-trash-info-header-function #'diredc-trash-info-header-default)
...
Load local configuration (not version controlled)
#+BEGIN_SRC emacs-lisp
  (if (file-exists-p (concat my/emacshome "local_post.el"))
      (load (concat my/emacshome "local_post.el")))
#+END_SRC
